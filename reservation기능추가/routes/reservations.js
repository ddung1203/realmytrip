...
router.get('/:id/reserve', needAuth, catchErrors(async (req, res, next) => {
  const reservation = await Reservation.findById(req.params.id);
  res.render('reservations/reserve', {reservation: reservation});
}));

router.get('/:id', catchErrors(async (req, res, next) => {
  const reservation = await Reservation.findById(req.params.id).populate('author');
  const questions = await Question.find({reservation: reservation.id}).populate('author');
  
  reservation.numReads++;    // TODO: 동일한 사람이 본 경우에 Read가 증가하지 않도록???

  await reservation.save();
  res.render('reservations/show', {reservation: reservation, questions: questions});
}));
...
router.post('/', needAuth, upload.single('img'), catchErrors(async (req, res, next) => {//몽고에서 고유의 아이디를 주기에......
  ...
  if (req.file) {
    ...
    reservation.img = "/images/uploads/" + filename;
  }
  await reservation.save();
...
router.post('/:id/questions', needAuth, catchErrors(async (req, res, next) => {
  const user = req.user;
  const reservation = await Reservation.findById(req.params.id);
