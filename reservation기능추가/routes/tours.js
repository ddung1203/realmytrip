...
router.get('/:id', catchErrors(async (req, res, next) => {
  ...
  const questions = await Question.find({tour: tour.id}).populate('author');
  tour.numReads++;    // TODO: 동일한 사람이 본 경우에 Read가 증가하지 않도록???

  await tour.save();
  res.render('tours/show', {tour: tour, questions: questions});
}));
...
router.post('/:id/questions', needAuth, catchErrors(async (req, res, next) => {
  ...
  req.flash('success', 'Successfully questioned');
